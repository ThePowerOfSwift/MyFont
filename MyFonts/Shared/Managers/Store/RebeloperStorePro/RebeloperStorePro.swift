...
extension RebeloperStore {
    ...
    static func verifyRenewablePurchaseLocaly(_ sufix: PurchaseSufix, completion: @escaping (Result<Bool, Error>) -> ()) {
        ...
        let productId = D_InfoPlistParser.getBundleId() + "." + sufix
        completion(.success(isSubscribed(subscriptionProductID: productId)))
        
    }
    
    static func verifySubscriptionsLocaly(_ sufixes: Set<PurchaseSufix>, completion: @escaping (Result<Bool, Error>) -> ()) {
        ...
        for productId in productIds {
            let isProductSubscribed = isSubscribed(subscriptionProductID: productId)
            if isProductSubscribed { subscribed = true }
        }
        completion(.success(subscribed))
    }
    
    fileprivate static func isSubscribed(subscriptionProductID: String) -> Bool {
        do {
